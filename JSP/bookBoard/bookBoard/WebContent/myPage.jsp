...
function enterkey(keyWord) {
    if (window.event.keyCode == 13) {
         ...
         // 엔터키가 눌렸을 때 실행할 내용
         location.href='SearchQueryMyBook.jsp?keyWord='+keyWord;
    }
}
</script>
...
<button type="button" class="btn btn-success pull-right" onclick="location.href='selMyProduct.jsp?stu_no=<%=stu_no%>'">판매 중인 책만 보기</button>
	<div class="text-center">
		...
		<%
						...
						%>
							<li>
						      <a href="queryMyBook.jsp?stu_no=<%=stu_no %>&pg=<%=startPage-1 %>" aria-label="Previous">
						...
						%>
									<li><a href="queryMyBook.jsp?stu_no=<%=stu_no %>&pg=<%=i %>"><%=i %></a></li>
						<%
								}
							}
						%>
						...
						%>
							...
							<li>
						      <a href="queryMyBook.jsp?stu_no=<%=stu_no %>&pg=<%=endPage+1 %>" aria-label="Next">
