...
<%
	...
	String sql = "SELECT * FROM BOOKINFO WHERE SORT >= " + start + " AND SORT <= " + end + " AND selstat = '"+selStat+"' and bookname like'%"+keyWord+"%' ORDER BY SORT";
	...
	stat1 = conn.createStatement();
		...
		String sqlCount = "SELECT COUNT(*) FROM bookinfo where selstat='"+selStat+"' and bookname like '%"+keyWord+"%'";
		...
		int sort = 1;
		String sqlSort = "SELECT tradenum FROM bookinfo WHERE selstat = '"+selStat+"' and bookname like '%"+keyWord+ "%' ORDER BY tradenum DESC";
		rs = stat1.executeQuery(sqlSort);
		
		
		while(rs.next()){
			int stepNum = rs.getInt(1);
			String sqlColumnUpdate = "UPDATE bookinfo SET SORT = " + sort + " WHERE tradenum = " + stepNum+ " and selstat='"+selStat+"' and bookname like'%"+keyWord+"%'";
	...
	RequestDispatcher rd = request.getRequestDispatcher("SearchSelProductView.jsp");
