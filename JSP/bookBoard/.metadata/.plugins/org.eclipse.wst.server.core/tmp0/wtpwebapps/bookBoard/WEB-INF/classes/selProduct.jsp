...
<%
	...
	String sql = "SELECT * FROM BOOKINFO WHERE SORT >= " + start + " AND SORT <= " + end + " AND selstat = '"+selStat+"' ORDER BY SORT";
	...
	stat1 = conn.createStatement();
		...
		String sqlCount = "SELECT COUNT(*) FROM bookinfo where selstat='"+selStat+"'";
		...
		int sort = 1;
		String sqlSort = "SELECT tradenum FROM bookinfo WHERE selstat = '"+selStat+"' ORDER BY tradenum DESC";
		rs = stat1.executeQuery(sqlSort);
		
		
		while(rs.next()){
			int stepNum = rs.getInt(1);
			String sqlColumnUpdate = "UPDATE bookinfo SET SORT = " + sort + " WHERE tradenum = " + stepNum+ " and selstat='"+selStat+"'";
	...
	RequestDispatcher rd = request.getRequestDispatcher("selProductView.jsp");
