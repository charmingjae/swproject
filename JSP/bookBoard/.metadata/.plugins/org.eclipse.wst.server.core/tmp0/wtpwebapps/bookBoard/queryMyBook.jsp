...
<%
	...
	String sql = "SELECT * FROM BOOKINFO WHERE SORT >= " + start + " AND SORT <= " + end + " and SELLER = '"+ seller + "' ORDER BY SORT";


		String sqlCount = "SELECT COUNT(*) FROM bookinfo where seller='"+seller+"'";
		...
		int sort = 1;
		String sqlSort = "SELECT tradenum FROM bookinfo where seller='"+seller+"' ORDER BY tradenum DESC";
		rs = stat1.executeQuery(sqlSort);
		
		while(rs.next()){
			int stepNum = rs.getInt(1);
			String sqlColumnUpdate = "UPDATE bookinfo SET SORT = " + sort + " WHERE tradenum = " + stepNum + " and seller='"+seller+"'";
