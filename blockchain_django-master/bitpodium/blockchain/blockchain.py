...
class Blockchain(object):
    ...
    def new_transaction(self, sender, recipient, amount):
        ...
        """
        conn = pymysql.connect(host='localhost', user='root', password='ckalswosla1', db='pm', charset='utf8')
        curs = conn.cursor()

        query = 'SELECT PRODUCT_NAME, NATION, COUNT FROM PRODUCT ORDER BY NO DESC LIMIT 1'
        # recipient = 'SELECT NATION FROM PRODUCT WHERE NO = 13'
        # amount = 'SELECT COUNT FROM PRODUCT WHERE NO =13'

        curs.execute(query)
        # curs.execute(recipient)
        # curs.execute(amount)
