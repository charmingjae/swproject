...
public class PublicChain {
	...
	public static void main(String[] args) {	
		...
		for(int idx = 0; idx<1; idx++) {
			if(idx == 0) {
				blockchain.add(new Block(idx, Integer.toString(idx+1)+"번째 블록", "0", "1", "2", 0));
				System.out.println((idx+1)+"번째 블록 마이닝 중 ,,");
				blockchain.get(idx).mineBlock(difficulty);
			}
			else {
				blockchain.add(idx, new Block(idx, Integer.toString(idx)+"번째 블록", blockchain.get(blockchain.size()-1).hash,  "1","1", 0));
				System.out.println((idx+1)+"번째 블록 마이닝 중 ,,");
				blockchain.get(idx).mineBlock(difficulty);
			}
		}
